...
def get_avatar(role):
    ...
    if "GPT" in provider:
        return "./photo/ai_logo_chatgpt_small_1.png"
    elif "deepseek" in provider:
        return "./photo/ai_logo_avatat.png"  # Update path as needed
    elif "Gemini" in provider:
        return "./photo/ai_logo_gemini_avatat.png"  # Update path as needed
...
def chat_gpt(user_prompt, system_prompt=""):
    ...
    #     color: #BBBBBB;
    #     padding: 5px 10px;  /* Top/Bottom padding, Left/Right padding */
    #     font-size: 12px;
    #     line-height: 1.2;
    #     min-height: 0px;    /* Reset default min-height */
    # }
    # </style>
    # """, unsafe_allow_html=True)
...
#     background-color: #4a4f90;   /* choose your hover color */
#     color: #ffffff;
#     cursor: pointer;
# }
...
st.markdown("""
    <style>
    /* 1. Target the button specifically to prevent text wrapping */
    div.stButton > button {
        background-color: #3b3f73 !important;
        ...
        /* FIX: Prevents text from breaking into parts/multiple lines */
        white-space: nowrap !important;
        word-break: keep-all !important;
        display: inline-block !important;
        width: auto !important;

        transition: background-color 150ms ease, color 150ms ease, transform 80ms ease !important;
    }

    /* 2. FIX: Hover effect (using higher specificity) */
    div.stButton > button:hover {
        background-color: #4a4f90 !important;
        color: #ffffff !important;
        border-color: #4a4f90 !important; /* Prevents default border color flicker */
    }

    /* 3. Optional: Ensure text remains white/visible when hovered */
    div.stButton > button:hover p {
        color: #ffffff !important;
    }

    /* 4. Optional: Pressed state */
...
with col_left:
    ...
    if mode == "Text Context":
        ...
        st.text_area(
            ...
            key="context_input_text",
            placeholder=(
                "Add your context here (Code, Emails, or Text)...\n\n"
                "Context Size Limits (Words):\n"
                "--------------------------------------------------\n"
                "• ChatGPT 5.2 (Thinking): 300,000 In | 96,000 Out\n"
                "• ChatGPT 5.2 (Instant): 96,000 In | 12,000 Out\n"
                "• Gemini 3 Flash: 780,000 In | 49,000 Out\n"
                "• DeepSeek Reasoner: 96,000 In | 48,000 Out\n"
                "• DeepSeek Chat: 96,000 In | 6,000 Out\n"
                "--------------------------------------------------\n"
    ...
    with col_mem1:
        ...
        chat_mode = st.pills(
            "Expertise Mode",
            ["Chatter", "Coder", "Consultant"],
            default="Consultant",  # Must match one of the options
            help="Chatter is for daily chat. Coder can produce structured code. Consultant answers in professional language."
        )

        # chat_mode = st.pills("Expertise Mode", ["Chatter", "Coder", "Consultant"], default="Pro", help="Chatter is for daily chat. Coder can produce structure code. Consultant answer in professional language.")
    with col_mem2:
